...
@property(nonatomic) NSDictionary<NSString *, id> *advertisementData;
    ...
    void (^tryService)(int) = GNCRecursiveIntHandler(^(GNCIntHandler tryService, int serviceIndex) {
      ...
      if (serviceIndex == peripheral.services.count) {
        ...
        if (_gattCharacteristicValues.count > 0) {
          NSData *charValue = [[_gattCharacteristicValues allValues] objectAtIndex:0];
          [strongSelf completeGATTReadWithData:charValue forPeripheralInfo:peripheralInfo];
        }
        return;
      }
